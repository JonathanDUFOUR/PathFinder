...
/*   Updated: 2021/04/16 19:26:28 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_cell_lst *options)
{
	...
	printw("Left neighbour -> s_dist for matrix[%u] == %d\n",
		idx[I_LOW] - 1, s_dist);
	e_dist = pf_calc_dist(dim, idx[I_END], idx[I_LOW] - 1);
	printw("Left neighbour -> e_dist for matrix[%u] == %d\n",
		idx[I_LOW] - 1, e_dist);
	matrix[idx[I_LOW] - 1] = s_dist + e_dist;
	pf_lst_add_back(options, idx[I_LOW] - 1);
}
